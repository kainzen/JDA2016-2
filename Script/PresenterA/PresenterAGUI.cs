...
public class PresenterAGUI : MonoBehaviour {
    ...
    private PresenterAWebViewEvents 	browserEventHandler; 
	...
	private UnityWebCore.JSCallbackDelFunc 			jsCallbackDelFunc;	
		
	//Classe de Controle e Modelo
	PresenterAControl presACrtl;
	PresenterModel presModel;
	WebIntercomGUI webInGUI;
	...
	void Start () {
		presACrtl = GameObject.FindObjectOfType(typeof(PresenterAControl)) as PresenterAControl;
		webInGUI = GameObject.FindObjectOfType(typeof(WebIntercomGUI)) as WebIntercomGUI;
						
		presModel = new PresenterModel();
		Debug.Log("Presenter: Request");
		presACrtl.setModel(presModel);	
		
		browserEventHandler = GetComponent<PresenterAWebViewEvents>();
    ...
    {		
		...
		if(m_bForceOpenGL && !Application.isEditor)
		{
			...
			try {
				...
				webInGUI.SystemMsg(ex.ToString());
			}
		}	
		else
		{
		...
		jsCallbackDelFunc = new UnityWebCore.JSCallbackDelFunc(this.onJSCallbackDelFunc);
		...
		UnityWebCore.AddJSCallback(m_TextureID, "JSCallbackDelFunc", jsCallbackDelFunc);
    ...
    void OnGUI() {
		...
		if(cameraA.enabled) {			
			...
			try {
				...
				webInGUI.SystemMsg("Erro ocorrido...");
				webInGUI.SystemMsg(ex.Message);
				webInGUI.SystemMsg("...favor reiniciar a aplicativo.");
			}
			GUILayout.EndArea();
	...
	public void Destroy()
    {
        try
        {
            if (m_TextureID != 0)
            {
				...
				Destroy(m_texture);
                GetComponent<PresenterAWebViewEvents>().interactive = false;
                m_TextureID = 0;
	...
	public void onJSCallbackDelFunc(string arguments)
	{
		webInGUI.SystemMsg(arguments);		
	}
}
